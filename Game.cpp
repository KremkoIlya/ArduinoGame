...
byte dracon[8] = {
 0b01110, 0b01010, 0b01110, 0b00100, 0b01110, 0b10101, 0b01010, 0b11011
};
byte derevo[8] = {
...
void setup() {
 ...
 pinMode (A2, INPUT); // подключаем кнопку дальнего прышка
 analogWrite(A1, LOW);
 analogWrite(A2,LOW); 
 ...
 LCD.backlight();              // включение подсветки дисплея
  ...
  pinMode(13, OUTPUT); // объявляем пин 13 как выход светодиод
...
void loop() {
  ...
  while (x > 0) {
    ...
    if (x == 4 && y == d) {
      digitalWrite(12, HIGH); // зажигаем светодиод  когда он умрет
